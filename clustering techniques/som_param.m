function som_param(input_data,dataset_id,start,numC,attr)    
%% Input arguments
%input_data->
...
%
%% Description
%This function makes experiments trying all the possible parameters of the
%clustering algorithm Som and computes the values of the 6 metrics for 
%the value of variable-start till the value of variable-numC. To find more
%reliable results we run all the algorithms that have some randomness factor 
%10 times and take the average value for the metrics. The results are saved 
%into .xls files

for i=start:numC

    for k=1:10
        [idx_temp,ci_temp] = som(input_data,i,3,'hextop','dist'); 
    ...
    for k=1:10
        [idx_temp,ci_temp] = som(input_data,i,3,'hextop','mandist'); 
        while length(unique(idx_temp))~=i 
            [idx_temp,ci_temp] = som(input_data,i,3,'hextop','mandist');
        end 
        while length(unique(idx_temp))~=i 
            [idx,ci] =  kmeanspp(input_data', i);
        end
        j_temp(k)=J(input_data,idx_temp,ci_temp);
        mia_temp(k) = MIA(input_data,idx_temp,ci_temp);
        cdi_temp(k) = CDI(input_data,idx_temp,ci_temp);
    ...
    for k=1:10
        [idx_temp,ci_temp] = som(input_data,i,3,'gridtop','dist'); 
        while length(unique(idx_temp))~=i 
            [idx_temp,ci_temp] = som(input_data,i,3,'gridtop','dist');
        end 
        j_temp(k)=J(input_data,idx_temp,ci_temp);
    ...
    for k=1:10
        [idx_temp,ci_temp] = som(input_data,i,3,'hextop','linkdist'); 
        while length(unique(idx_temp))~=i 
            [idx_temp,ci_temp] = som(input_data,i,3,'hextop','linkdist');
        end 
        j_temp(k)=J(input_data,idx_temp,ci_temp);
    ...
    for k=1:10
        [idx_temp,ci_temp] = som(input_data,i,3,'hextop','boxdist'); 
        while length(unique(idx_temp))~=i 
            [idx_temp,ci_temp] = som(input_data,i,3,'hextop','boxdist');
        end 
        j_temp(k)=J(input_data,idx_temp,ci_temp);
...
if attr==0
    filename = sprintf('som%d.xlsx',dataset_id);
    xlswrite(filename,all_metrics,1,'A1:E54')

    filename = sprintf('som%d_j.xlsx',dataset_id);
    xlswrite(filename,j(start:numC,:),1,'A1:F9')
else
    filename = sprintf('som_attr%d.xlsx',dataset_id);
    xlswrite(filename,all_metrics,1,'A1:E54')

    filename = sprintf('som_attr%d_j.xlsx',dataset_id);
    xlswrite(filename,j(start:numC,:),1,'A1:F9')
end
